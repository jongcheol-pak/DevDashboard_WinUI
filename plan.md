...
| 상태 아이콘 | 통과 | **라운드 사각형**(약 22px) 파란 채움 + **흰색** ✓ | 이미지1 |
| 상태 아이콘 | 실패 | **라운드 사각형** 호박 채움 + **흰색** ✕ | 이미지1(재지적 시 확인) |
| 상태 아이콘 | 미실행 | **속이 빈 원** — 회색 테두리만, 배경·글리프 없음 | 이미지1 + 사용자 재지적(2026-07-21, D3 이력) |
...
- **D3 (상태 아이콘)**: 22×22. 통과·실패는 `CornerRadius=6` 라운드 사각형에 상태색 **채움 + 흰 글리프**(테두리 두께 0 — Border 배경이 테두리 안쪽에만 그려져 크기가 줄기 때문), 미실행은 **`CornerRadius=11` 원 + 회색 테두리 1.5 + 배경 투명 + 글리프 없음**.
  - *이력*: 최초에는 Q1 답변대로 세 상태 모두 라운드 사각형 + 미실행에 ○ 글리프로 구현했으나, 렌더 확인 결과 **사각 테두리 안에 원 글리프가 겹쳐 시안과 크게 달라 보였다**(사용자 재지적 2026-07-21). 시안(이미지1)의 미실행은 속이 빈 원 하나이므로 **시안 기준으로 정정**. *Source*: 사용자 제공 시안 이미지1 + 렌더 대조(2026-07-21).
