...
- [ ] T2 — 테스트 영속화(값 마이그레이션 + Method 컬럼)
- [ ] T3 — TestPageViewModel
- [ ] T5 — 테스트 등록/편집·메모 다이얼로그 (T4보다 먼저 구현)
- [ ] T4 — 테스트 전체 페이지 View(스위트/통계/통과율 restyle)
- [ ] T7 — 작업 카드 배지 매핑 갱신 + 연결 테스트 기본상태 (T6보다 먼저 — 구 상수 참조 이전)
