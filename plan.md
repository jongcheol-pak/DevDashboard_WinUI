...
- **프로젝트별 카드 색을 `Palette.xaml` 토큰으로 만드는 것** — 카드 색은 프로젝트마다 다른 **데이터**이지 팔레트 토큰이 아니다. ※ 화면 공통 UI 토큰(예: 카드 hover 테두리 `CardStrokeColorHover`)은 이 제외 대상이 **아니며** 이 레포 관례대로 `Palette.xaml` Default에 추가한다 — 함정 4는 Light/HighContrast 딕셔너리 신설 금지이지 색 추가 금지가 아니고, 이 레포는 "원시 hex 브러시는 `Palette.xaml`에만"을 예외 없이 지켜왔다(T4 quality 리뷰 M1로 확인).
...
## F-8 인계 목록 (렌더·실동작 육안 확인 필요 — 완료 선언 보류 사유)

1. **앱이 정상 기동하는지** — `Projects.HeaderColor` 마이그레이션이 실제 DB에서 통과하는지(T1 acceptance 7. 화이트리스트 누락 시 기동 즉시 예외였으므로 실행 확인이 유일한 검증)
2. 카드가 **색상 헤더 밴드 + 아바타**로 보이는지, 색을 지정하지 않은 기존 카드들이 **서로 다른 색**으로 나뉘는지
3. 프로젝트 설정에서 **색을 골라 저장 → 카드 반영 → 앱 재시작 후에도 유지**, "자동"으로 되돌리기
4. **아이콘이 있는 프로젝트는 아이콘**, 없으면 이니셜이 아바타에 나오는지 / 밝은 헤더 색에서 흰 이니셜 대비가 충분한지
5. `…` 메뉴 7항목 각각의 동작 — 특히 **터미널 ▸ PowerShell**(축약 전 유일 진입점이었던 경로)과 삭제 항목의 위험색
6. 실행 버튼 클릭(실행)·우클릭(관리자 권한), 도구 경로 오류·관리자 권한 표시가 버튼 안에 보이는지
