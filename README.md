...
### 프로젝트 카드 대시보드
- 프로젝트별 카드 등록 (이름·설명·아이콘·경로·기술 스택 태그·카테고리·**카드 색상**)
- 카드마다 **색상 헤더 밴드 + 아바타** — 색을 고르지 않으면 프로젝트 이름에 따라 자동 배정되고, 아이콘을 등록하면 아이콘이 이니셜 대신 표시됩니다
- **그룹 탭**으로 프로젝트 분류 (추가 / 이름 변경 / 삭제 / 좌우 스크롤) + 탭별 **프로젝트 개수 배지**
- 헤더 검색창으로 카드 즉시 필터링 (개수 배지도 검색 결과를 따라갑니다)
- 정렬 기준 선택: 이름순 / 카테고리순 / 최근 등록순
- 카드 드래그&드롭 정렬 + 상단 핀 고정
- 창 크기에 따라 카드 폭이 늘어나는 가변 그리드
...
- 관리자 권한 실행 옵션
- 카드에는 **개발 도구 실행 버튼 + 작업 + 테스트**만 노출하고, 나머지(폴더 열기·터미널·Git 상태·작업 기록·프로젝트 설정·삭제)는 **"더 보기"(…) 메뉴**에 모아 두었습니다
- 카드별 **커맨드 스크립트 슬롯 4개** — 셸 타입·아이콘·작업 폴더·완료 후 자동 종료 설정.
  설정된 슬롯만 표시되고 끝의 **＋** 버튼으로 새 슬롯을 추가합니다
...
2. 프로젝트 설정 다이얼로그에서 정보를 입력합니다.
   ...
   - **카드 헤더 색상**: 카드 상단 밴드와 아바타 색. "자동"으로 두면 프로젝트 이름에 따라 배정됩니다
   - **경로**: 프로젝트 루트 디렉터리
...
3. 등록된 카드는 개발 도구 실행 버튼으로 IDE를 열고(우클릭하면 관리자 권한),
   **"더 보기"(…) 메뉴**에서 폴더 열기·터미널·Git 상태·작업 기록·프로젝트 설정·삭제를 할 수 있습니다.
   핀 고정과 삭제는 카드 헤더의 아이콘으로도 가능합니다.
