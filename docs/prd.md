...
- **FR-T6 (Should)**: 새 작업 다이얼로그의 "테스트 추가" 토글 — 작업을 테스트 목록에도 등록(작업↔테스트 연결). (Phase 3 테스트 연결과 연동)
  - *2026-07-21 배선 정정*: 링크 테스트를 고정 "작업" 스위트에 넣던 것을 **작업의 카테고리 이름과 같은 테스트 스위트**에 넣도록 변경. FR-T8 배지가 "같은 이름의 테스트 카테고리"를 찾는데 링크 테스트가 항상 "작업" 스위트로 가 이름이 어긋나 배지가 사실상 뜨지 않던 것을 해소(FR-T8 배선 완성). 카테고리가 빈 "미분류" 작업은 배지 대상이 아니므로 기존 "작업" 스위트로 폴백.
- **FR-T7 (Could)**: 담당자(who) 필드 표시/편집.
- **FR-T8 (Should)**: 칸반 카테고리 그룹 헤더에 해당 카테고리의 테스트 통과율 배지 표시(예: `✓ 통과율 67% · 4건`). 같은 이름의 테스트 카테고리가 없으면 미표시. (Phase 3와 연동)
  - *2026-07-20 변경*: 원안은 "작업 카드에 연결 테스트 상태 배지"였으나, 디자인 시안이 카드 배지 대신 **카테고리 그룹 단위 통과율 배지**를 쓴다(카드는 제목·우선순위·설명·날짜만). 시안 정합 재구성에 맞춰 요구를 갱신.
...
- **FR-E4 (Should)**: 테스트↔작업 연결(연결 배지·링크), 칸반 카테고리 그룹 통과율 배지 배선(FR-T8). *(2026-07-20: FR-T8 갱신에 맞춰 "작업 카드 배지" → "카테고리 그룹 통과율 배지"로 문구 수정)*
...
> 2026-07-22 사용자 요청으로 §7 Out of Scope에서 **해제**되어 편입된 영역이다(D-9). 시안 원본 `docs/design/DevDashboard Redesign.dc.html`의 대시보드 분기(`:332~404`)·카드 메뉴(`:450~487`)가 기준.

- **FR-D1 (Must)**: 프로젝트 카드를 시안 구조로 재구성한다 — 색상 헤더 밴드(62px) + 이니셜/아이콘 아바타 + 이름 + 핀·삭제, 본문은 설명 1줄·태그·액션 행.
- **FR-D2 (Must)**: 프로젝트별 카드 색상을 도입한다 — 도메인 필드 영속(하위호환 마이그레이션), 미지정 시 이름 기반 자동 배정, 프로젝트 설정 다이얼로그에서 팔레트로 선택.
- **FR-D3 (Must)**: 카드 액션을 축약한다 — 개발도구 실행 버튼 + 작업 + 테스트만 노출하고 나머지(폴더·터미널·Git 상태·작업 기록·설정·삭제)는 "더 보기" 메뉴로 이동. 하단 스크립트 슬롯은 설정된 것만 표시 + 추가 버튼.
- **FR-D4 (Should)**: 카드 그리드 폭을 창 크기에 따라 가변화하고, 그룹 탭에 프로젝트 개수 배지를 표시한다.
...
| 6 | 대시보드 카드 | 카드 재구성·카드 색상·액션 축약(더 보기 메뉴)·그리드 가변폭·그룹 탭 배지 | 0 |
...
- **D-9 대시보드 카드 리디자인 편입 (2026-07-22)**: ✅ **§7 Out of Scope에서 해제하고 Phase 6으로 편입** — 사용자 요청("메인 카드목록 화면을 시안대로 수정")에 따라 카드 재구성·카드 색상·액션 축약을 정식 요구(FR-D1~D4)로 등재했다. 요구 변경은 PRD → plan → 코드 순서(§ 서두 규약)를 지켜 코드 착수 전에 이 문서를 먼저 갱신했다.
