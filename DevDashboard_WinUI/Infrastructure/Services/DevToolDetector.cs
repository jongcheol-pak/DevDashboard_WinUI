...
/// <summary>
/// 알려진 개발 도구의 설치 여부를 파일 시스템에서 감지하는 서비스.
/// 반복 탐색 방지를 위해 감지 결과를 메모리에 캐싱합니다.
/// </summary>
internal static class DevToolDetector
{
    ...
    /// <summary>
    /// 실행 파일이 존재하는 도구만 반환합니다.
    /// 이전 감지 결과를 캐시하여 재사용합니다.
    /// </summary>
    public static List<ExternalTool> DetectInstalledTools()
    {
        ...
        lock (_lock)
        {
            // 이중 확인 잠금(Double-Checked Locking): lock 진입 후 재확인
