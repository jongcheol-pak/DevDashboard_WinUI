...
/// EnableWindow(false)는 WM_ENABLE → VisualState 전이 + MicaBackdrop 비활성 전환으로 깜빡임을 유발하므로 사용하지 않음.
/// </summary>
internal static class DialogWindowHost
{
    ...
    // 모달 입력 차단 — 중첩 다이얼로그 대응을 위해 깊이 카운터 사용
    private static int _modalDepth;
    private static Grid? _modalOverlay;

    // SubclassProc 델리게이트를 정적 필드로 보관하여 GC 수집 방지
